...
class LogsApi(val logsDir: Path, val maxLine: Int)(implicit val executionContext: ExecutionContext, val materializer: ActorMaterializer) extends EventMarshalling {
  ...
  val outFlow: Flow[Event, ByteString, NotUsed] = Flow[Event].map {
    event => ByteString(event.toJson.compactPrint)
  }
  ...
  def logFileSink(logId: String) = FileIO.toPath(logFile(logId), Set(CREATE, WRITE, TRUNCATE_EXISTING))
          ...
          entity(as[HttpEntity]) { entity =>
            onComplete(
              entity
                ...
                .run()) {
                case Success(IOResult(count, Success(Done))) => complete((StatusCodes.OK, LogReceipt(logId, count)))
